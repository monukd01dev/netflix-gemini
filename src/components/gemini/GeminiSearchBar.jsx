...
function GeminiSearchBar({ onSearch,inputRef }) {
    const { isFocused, setIsFocused, handleSubmit, query, setQuery, isDisabled, searchBarError,setSearchBarError } = useGeminiSearchBar(onSearch);
    ...
    return (
        <div className="w-full max-w-2xl md:max-w-3xl mx-auto px-4 sm:px-6">
            ...
            <div
                className={`relative rounded-full p-0.5 transition-all duration-500 ${isFocused
                ...
                >
                    ...
                    <div className="flex items-center gap-2 md:gap-3 shrink-0 text-zinc-400">
                        <button
                            type="submit"
                            disabled={isDisabled}
                            className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-all duration-300 ${isDisabled
